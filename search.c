...
int alphabeta_search(int alpha, int beta, int depth, move_set *moves,
                     move_set *variation) {
  move_set np_moves;
  ...
  for (i = 0; i < moves->nmoves; i++) {
    ...
    if (depth > 1) {
      ...
      val =
          -alphabeta_search(-beta, -alpha, depth - 1, &np_moves, &np_variation);
    }
    ...
    if (val >= beta) {
      killer_heuristic[b_get_ply_counter()] =
          moves->move[i]; /* annota la mossa che ha generato il taglio */
      return beta;
    }
    /* we found a better move */
    if (val > alpha) {
      alpha = val;
      moves->move[i].value++; /* in modo da riconoscere a livello superiore la
                                 mossa tra quelle di pari valore */
      variation->move[0] = moves->move[i]; /* inserisce la mossa in testa alla
                                              linea di gioco di questo ramo */
      for (j = 0; j < np_variation.nmoves;
           j++) /* e ci accoda la linea di gioco successiva alla mossa in testa
                 */
        variation->move[j + 1] = np_variation.move[j];
      variation->nmoves =
          np_variation.nmoves + 1; /* imposta la lunghezza della variazione */
    }
  }
...
void s_best_move(move *m, int output) {
  ...
  if (time_left) {
    if (level_moves)
      exact_time =
          time_left / (level_moves - (b_get_ply_counter() / 2) % level_moves) +
          level_inc / 2 - 20;
    else
  ...
  while (!abort_search) {
    ...
    nodes_per_second =
        (int)((float)(100 * (ab_nodes + q_nodes)) / u_stop_timer(start));
...
move *s_get_killer(int ply) { return &killer_heuristic[ply]; }

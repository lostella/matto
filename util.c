...
long u_start_timer() { return clock(); }

/* returns the time elapsed (in centiseconds) since the last startTimer call */
int u_stop_timer(long t0) { return (clock() - t0) * 100 / CLOCKS_PER_SEC; }

char *u_write_move(const move *m, char *s) {
  sprintf(s, "%c%c%c%c", (8 - (m->from % 16)) - 1 + 'a', (m->from / 16) + '1',
...
int u_parse_move(const char *s, move *m) {
  if (strlen(s) >= 4 && strlen(s) <= 5 && s[0] >= 'a' && s[0] <= 'h' &&
      s[1] >= '1' && s[1] <= '8' && s[2] >= 'a' && s[2] <= 'h' && s[3] >= '1' &&
      s[3] <= '8') {
